...
def process_operation(
        ...
        resource_bases,
        is_workflows=False):
    def create_operation(operation_mapping,
                         operation_payload,
                         operation_executor):
        ...
        if candidate_plugins:
            if len(candidate_plugins) > 1:
                raise exceptions.DSLParsingLogicException(
                    91, 'Ambiguous operation mapping. [operation={0}, '
                        'plugins={1}]'.format(operation_name,
                                              candidate_plugins))
            ...
            raise exceptions.DSLParsingLogicException(error_code,
                                                      error_message)
    ...
    if not is_workflows and isinstance(operation_mapping, list):
        return [
            create_operation(inner_op, operation_payload, operation_executor)
            for inner_op in operation_mapping]
    else:
        return create_operation(operation_mapping,
                                operation_payload,
                                operation_executor)
